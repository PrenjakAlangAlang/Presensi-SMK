...
<div class="flex justify-between items-center mb-6">
    <div>
        ...
        <p class="text-gray-600">Kelola data mata pelajaran dan penempatan siswa</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div class="flex items-center justify-between">
            <div>
                ...
                <h3 class="text-2xl font-bold text-gray-800 mt-1"><?php echo count($mataPelajaran); ?></h3>
            </div>
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200">
                    <th class="px-6 py-4 text-left text-sm font-medium text-gray-700">Nama Mata Pelajaran</th>
                    <th class="px-6 py-4 text-left text-sm font-medium text-gray-700">Tahun Ajaran</th>
                    <th class="px-6 py-4 text-left text-sm font-medium text-gray-700">Guru Pengampu</th>
                    <th class="px-6 py-4 text-left text-sm font-medium text-gray-700">Jadwal</th>
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <span class="font-medium text-gray-800"><?php echo htmlspecialchars($k->nama_mata_pelajaran); ?></span>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-gray-600"><?php echo htmlspecialchars($k->tahun_ajaran); ?></td>
                    <td class="px-6 py-4">
                        <?php if($k->guru_pengampu_nama): ?>
                            <div class="flex items-center space-x-2">
                                <div class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center">
                                    <i class="fas fa-chalkboard-teacher text-green-600 text-sm"></i>
                                </div>
                                <span class="text-gray-800"><?php echo htmlspecialchars($k->guru_pengampu_nama); ?></span>
                            </div>
                        <?php else: ?>
                    ...
                    <td class="px-6 py-4 text-gray-600">
                        <?php 
                        $jumlahSiswa = count($mataPelajaranModel->getSiswaInMataPelajaran($k->id));
                    ...
                    <td class="px-6 py-4">
                        <div class="flex space-x-2">
                                                    <button 
                                                        ...
                                                        data-nama_mata_pelajaran="<?php echo htmlspecialchars($k->nama_mata_pelajaran, ENT_QUOTES); ?>"
                                                        data-tahun_ajaran="<?php echo htmlspecialchars($k->tahun_ajaran, ENT_QUOTES); ?>"
                                                        data-guru_pengampu="<?php echo $k->guru_pengampu; ?>"
        ...
        <form method="POST" action="index.php?action=admin_create_kelas">
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <select name="guru_pengampu" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4">
        ...
        <form method="POST" action="index.php?action=admin_update_kelas">
            ...
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Nama Mata Pelajaran</label>
                    <input type="text" name="nama_mata_pelajaran" id="edit_kelas_nama" required 
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Tahun Ajaran</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Guru Pengampu</label>
                    <select name="guru_pengampu" id="edit_kelas_wali" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition">
                        <option value="">Pilih Guru Pengampu</option>
                        <?php foreach($guru as $g): ?>
                            <option value="<?php echo $g->id; ?>"><?php echo htmlspecialchars($g->nama); ?></option>
                        <?php endforeach; ?>
...
<div id="kelolaSiswaModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl mx-4 max-h-[90vh] overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            <h3 class="text-xl font-semibold text-gray-800">Kelola Siswa Mata Pelajaran</h3>
            <p class="text-gray-600 text-sm" id="modalKelasTitle"></p>
        </div>
        <div class="p-6 overflow-y-auto max-h-96">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <!-- Daftar Siswa di Mata Pelajaran -->
                <div>
                    <h4 class="font-semibold text-gray-800 mb-4">Siswa dalam Mata Pelajaran</h4>
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        ...
                        <button onclick="tambahSiswaKeKelas()" class="w-full bg-green-600 hover:bg-green-700 text-white py-2 px-4 rounded-lg transition-colors">
                            <i class="fas fa-plus mr-2"></i>Tambah ke Mata Pelajaran
                        </button>
                    </div>
                </div>
            </div>
...
function openEditKelasModal(button) {
    ...
    var nama = button.getAttribute('data-nama_mata_pelajaran');
    var tahun = button.getAttribute('data-tahun_ajaran');
    var wali = button.getAttribute('data-guru_pengampu');
...
function kelolaSiswa(kelasId) {
    ...
    const selectedKelas = mataPelajaran.find(k => k.id == kelasId);
    
    if (selectedKelas) {
        document.getElementById('modalKelasTitle').textContent = `Mata Pelajaran: ${selectedKelas.nama_mata_pelajaran}`;
    }
    
    // Load siswa dalam mata pelajaran (from server)
    loadSiswaDalamKelas(kelasId);

    // Load siswa yang tersedia
    loadSiswaTersedia();
    ...
    fetch(`index.php?action=admin_get_siswa_kelas&kelas_id=${kelasId}`)
        ...
        .then(data => {
            if (!data || data.length === 0) {
                container.innerHTML = '<p class="text-gray-500 text-center py-4">Belum ada siswa di mata pelajaran ini</p>';
                return;
            }
...
function tambahSiswaKeKelas() {
    ...
    .then(resp => {
        if (resp.success) {
            showNotification('success', 'Siswa berhasil ditambahkan ke mata pelajaran!');
            loadSiswaDalamKelas(currentKelasId);
            loadSiswaTersedia();
            select.value = '';
        } else {
            showNotification('error', 'Gagal menambahkan siswa ke mata pelajaran');
        }
    })
    .catch(() => showNotification('error', 'Gagal menambahkan siswa ke mata pelajaran'));
}
...
function hapusSiswaDariKelas(siswaId) {
    if (confirm('Apakah Anda yakin ingin menghapus siswa dari mata pelajaran ini?')) {
        ...
        .then(resp => {
            if (resp.success) {
                showNotification('success', 'Siswa berhasil dihapus dari mata pelajaran!');
                loadSiswaDalamKelas(currentKelasId);
                loadSiswaTersedia();
            } else {
                showNotification('error', 'Gagal menghapus siswa dari mata pelajaran');
            }
        })
        .catch(() => showNotification('error', 'Gagal menghapus siswa dari mata pelajaran'));
    }
}

function deleteKelas(kelasId) {
    if (confirm('Apakah Anda yakin ingin menghapus mata pelajaran ini? Semua data siswa dalam mata pelajaran akan terpengaruh.')) {
