...
class PresensiSesiModel {
    ...
    public function createSession($mata_pelajaran_id, $guru_id) {
        ...
        $this->db->query('INSERT INTO presensi_mapel_sesi (mata_pelajaran_id, guru_id, waktu_buka, status) VALUES (:mata_pelajaran_id, :guru_id, NOW(), "open")');
    ...
    public function closeSession($mata_pelajaran_id, $guru_id) {
        ...
        $this->db->query('UPDATE presensi_mapel_sesi SET waktu_tutup = NOW(), status = "closed" WHERE mata_pelajaran_id = :mata_pelajaran_id AND status = "open"');
        $this->db->bind(':mata_pelajaran_id', $mata_pelajaran_id);
    ...
    public function getActiveSessionByKelas($mata_pelajaran_id) {
        ...
        $this->db->query('SELECT * FROM presensi_mapel_sesi WHERE mata_pelajaran_id = :mata_pelajaran_id AND status = "open" LIMIT 1');
        $this->db->bind(':mata_pelajaran_id', $mata_pelajaran_id);
    ...
    public function getSessionsByKelas($mata_pelajaran_id) {
        ...
        $this->db->query('SELECT * FROM presensi_mapel_sesi WHERE mata_pelajaran_id = :mata_pelajaran_id ORDER BY waktu_buka DESC');
        $this->db->bind(':mata_pelajaran_id', $mata_pelajaran_id);
    ...
    public function getSessionById($id) {
        ...
        $this->db->query('SELECT * FROM presensi_mapel_sesi WHERE id = :id LIMIT 1');
        $this->db->bind(':id', $id);
        return $this->db->single();
    }
}
